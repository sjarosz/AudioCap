...
fileprivate extension AudioObjectID {
    ...
    static func translatePIDToProcessObjectID(pid: pid_t) throws -> AudioObjectID {
        ...
        let err = AudioObjectGetPropertyData(.system, &address, UInt32(MemoryLayout<pid_t>.size), &processID, &size, &objectID)
        guard err == noErr else { throw RecordingError.general("Failed to translate PID \(pid) to AudioObjectID: \(err)") }
        guard objectID.isValid else { throw RecordingError.general("Translated AudioObjectID for PID \(pid) is invalid") }
        return objectID
    }
